...
import os
import json  # ← ВАЖНО: добавьте эту строку!
...
# ============================================================
# 2. ПОДКЛЮЧЕНИЕ К FIREBASE (исправлено!)
# ============================================================
if not firebase_admin._apps:
    key_loaded = False
    
    # СПОСОБ 1: Для Streamlit Cloud — читаем из Secrets
    try:
        key_dict = json.loads(st.secrets["firebase_key"])
        cred = credentials.Certificate(key_dict)
        firebase_admin.initialize_app(cred)
        key_loaded = True
        st.success("✅ Firebase подключён через Secrets")
    except Exception as e:
        st.warning("⚠️ Secrets не найдены, пробую локальный файл...")
    ...
    if not key_loaded:
        ...
        if os.path.exists(KEY_PATH):
            try:
                ...
                firebase_admin.initialize_app(cred)
                key_loaded = True
                st.success("✅ Firebase подключён через локальный файл")
            except Exception as e:
                st.error(f"❌ Ошибка инициализации Firebase: {e}")
                st.stop()
        else:
            st.error("❌ Файл serviceAccountKey.json не найден!")
            st.info("💡 Если вы на Streamlit Cloud — добавьте ключ в Secrets (настройки приложения)")
            st.stop()
